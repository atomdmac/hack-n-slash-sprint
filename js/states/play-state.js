	...
	this.setup = function (options) {
		...
		(function () {
			...
			function addgamepad(gamepad) {
				/*if (!players[gamepad.index].gamepad) {
					players[gamepad.index].setGamepad(gamepad);*/
				if (!players[0].gamepad) {
					players[0].setGamepad(gamepad);
		...
		/*
		// Listen for gamepads, Firefox only
		window.addEventListener("gamepadconnected", function(e) {
			var gp = navigator.getGamepads()[e.gamepad.index];
			players[e.gamepad.index].setGamepad(gp);
